<?php $user = get_user_by( 'id', get_current_user_id() ); 
?>

 	<aside id="tophome"> 
		<div class="container hidden-xs">							
			<ul class="option-menu list-unstyled list-inline col-md-12 col-sm-12 pull-right text-left">
				<div  class="col-sm-12 col-xs-4 text-left" >
					<a class="atras"
						id="btn-linkprev" href="<?php echo get_home_url() ?>/?source=<?php echo get_source_url(); ?>">
						<i class="fa fa-chevron-left" aria-hidden="true"></i>
						ATRÁS
					</a>
				</div>

				<div style="float:right;width:90%; margin-top: -3%;" class="col-xs-8 col-md-12 text-center">   
					<label style="text-align: center; color: #000000; font-size: 25px; font-family: GothanMedium_regular; ">Cargar Orden
				    </label>
				</div>  
			</ul>
		</div>	


		<div class="container visible-xs hidden-sm hidden-md hidden-lg">							
			<ul class="option-menu list-unstyled list-inline col-xs-12 col-md-6  text-left">
					<div  class="col-sm-12 col-xs-4 text-left" >
						<a class="btn-kmibox-white"
							id="btn-linkprev" href="<?php echo get_home_url() ?>/">
							<i class="fa fa-chevron-left" aria-hidden="true"></i>
							ATRÁS
						</a>
					</div>

					<div style="float:left;width:50%;">    
						<label style="text-align: right;color: #000000; font-size: 16px; font-family: GothanMedium_regular; margin-right: -36%;">Cargar Orden
					    </label>
					</div>  
			</ul>
		</div>
		<div class="container visible-sm hidden-xs hidden-md hidden-lg">							
			<ul class="option-menu list-unstyled list-inline col-xs-12 col-md-6 hidden-sm text-left">
				<div  class="col-sm-12 col-xs-4 text-left" >
					<a class="atras"
						id="btn-linkprev" href="<?php echo get_home_url() ?>/">
						<i class="fa fa-chevron-left" aria-hidden="true"></i>
						ATRÁS
					</a>
				</div>

				<div style="float:left;width:50%;">    
					<label style="text-align: center;color: #000000;font-size: 18px;margin-left: 60%;margin-right: -90%;font-family: GothanMedium_regular;">Registro de Usuario
				    </label>
				</div>  
			</ul>
		</div>			
	</aside>



